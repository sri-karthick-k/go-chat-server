...
// 	db_url := "sri:Spartan03@/chatDBTest?charset=utf8"
...
// 			c.JSON(http.StatusInternalServerError, gin.H{"error": "Please Enter username, password and email"})
// 			return
// 		}
// 		// Hash the password
...
// 			insertStmt, _ := db.Prepare("INSERT INTO Users (username, email, password) VALUES (?, ?, ?)")
// 			_, err = insertStmt.Exec(username, email, hashedPassword)
// 			if err != nil {
...
// 		c.JSON(200, gin.H{
// 			"user": User{Id: id, Username: username, Email: email, Password: string(hashedPassword)},
// 		})
// 	})
...
// 		rows, err := db.Query("SELECT id, body, media, modified, sender_id, receiver_id FROM Messages ORDER BY modified ASC")
// 		if err != nil {
...
// 			return
// 		}
// 		fmt.Println(msgData.SenderUsername)
// 		var senderId, receiverId int
// 		err := db.QueryRow("SELECT id FROM Users WHERE username=?", msgData.SenderUsername).Scan(&senderId)
// 		if err != nil {
// 			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error finding sender"})
// 			return
// 		}
// 		err = db.QueryRow("SELECT id FROM Users WHERE username=?", msgData.ReceiverUsername).Scan(&receiverId)
// 		if err != nil {
// 			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error finding receiver"})
// 			return
// 		}

// 		insertStmt, _ := db.Prepare("INSERT INTO Messages (body, media, modified, sender_id, receiver_id) VALUES (?, ?, ?, ?, ?)")
// 		_, err = insertStmt.Exec(msgData.Content, nil, time.Now(), senderId, receiverId)
// 		if err != nil {
// 			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error inserting message"})
// 			return
// 		}
...
// 			return
// 		}
// 		// m.Broadcast([]byte(msgData.Content))
// 		m.BroadcastFilter([]byte(msgData.Content), func(q *melody.Session) bool {
